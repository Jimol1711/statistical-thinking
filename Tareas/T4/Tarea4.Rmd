...
```{r, eval=FALSE}
for (n in c(1, 2, 4, 8)) {
  # Sumar las mordidas en los primeros n meses
  W <- sum(dataMordidas[, cols[1:n]])
  
  # Calcular el número total de intentos
  L <- n * 250 - W
  
  # Ajustar el modelo utilizando el método de Laplace
  res <- quap(
    alist(
      W ~ dbinom(size = n * 250, prob = p),  # Likelihood binomial
      p ~ dunif(0, 1)                        # Prior uniforme para p
    ),
    data = list(W = W),  # Pasar los datos (W)
    start = list(p = 0.3)  # Valor inicial explícito para p
  )
  
  # Obtener el resumen del modelo
  df <- precis(res)
  
  # Graficar la distribución posterior aproximada
  curve(
    dnorm(x, mean = df$mean, sd = df$sd),
    from = 0, to = 1, lty = 2, add = FALSE,
    main = paste("Distribución Posterior para", n, "meses"),
