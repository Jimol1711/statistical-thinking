...
for (n in c(1, 2, 4, 8)) {
  ...
  # Definir la variable W como el número total de mordidas en los primeros n meses
  W <- sum(dataMordidas[, cols[1:n]])
  
  # Calcular el número total de oportunidades (n*250 - mordidas)
  L <- n * 250 - W
  
  # Ajustar el modelo usando el método de Laplace
  res <- quap(
    alist(
      W ~ dbinom(size = n * 250, prob = p),  # Likelihood binomial
      p ~ dunif(0, 1)  # Prior uniforme para la probabilidad
    ),
    data = list(W = W, L = L)
  )
  ...
  # Graficar la distribución posterior usando una curva normal
  curve(dnorm(x, df$mean, df$sd), lty=2, add=FALSE, main = paste("Distribución Posterior para", n, "meses"),
